...
from CowMusic import YouTube, app
from CowMusic.core.call import Cow
from CowMusic.misc import SUDOERS, db
from CowMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from CowMusic.utils.decorators.language import languageCB
from CowMusic.utils.formatters import seconds_to_min
from CowMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from CowMusic.utils.stream.autoclear import auto_clean
from CowMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Cow.pause_stream(chat_id)
        ...
        await Cow.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Cow.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Cow.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Cow.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Cow.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
